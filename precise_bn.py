...
from data_utils import TrueUnsupervisedDataset
...
class _MeanOfBatchVarianceEstimator:
    ...
    def __init__(self, mean_buffer: torch.Tensor,
                 var_buffer: torch.Tensor) -> None:
...
def update_bn_stats(
    ...
    def get_bn_batch_size_hook(
        ...
        ), "BN layers reused this is bad and now you cant unhook your models hooks :))"
        x = input[0]
        assert isinstance(
            x, torch.Tensor
    ...
    hooks_to_remove = [
        bn.register_forward_pre_hook(get_bn_batch_size_hook) for bn in
        bn_layers
    ]
...
def get_bn_modules(model: nn.Module) -> List[nn.Module]:
    ...
    bn_layers = [
        m for m in model.modules() if m.training and
                                      isinstance(m, BN_MODULE_TYPES) and
                                      list(m.parameters())[0].requires_grad
    ]
    return bn_layers


def make_dl_infinite(dl):
    """
    Creates an infinite repeating dataloader
    """
    generator = iter(dl)
    while True:
        try:
...
def fix_model_bn_stats_on_dataset(model, dataset, batch_size,
                                  num_workers=0,
                                  prefetch_factor=2,
                                  device="cpu"):
    """
    Make the means and variances of the model's BN layers be the true mean
    variances in the given dataset
    """
    def collate_then_transfer_to_device(data):
        res = default_collate(data)
        return res.to(device)

    dl = DataLoader(TrueUnsupervisedDataset(dataset), batch_size,
                    shuffle=True,
                    num_workers=num_workers,
                    prefetch_factor=prefetch_factor,
                    collate_fn=collate_then_transfer_to_device)
    dl = make_dl_infinite(dl)
    model.train()
    update_bn_stats(model, dl, progress="tqdm")
